...
const Card = ({ data }) => {
  ...
  return (
    <>
      {data.isPopular && (
        <div className="relative flex flex-col font-rubik items-center justify-center max-md:justify-start max-md:gap-5 border-4 rounded-2xl w-[187px] h-[261px] max-md:w-full max-md:h-full max-md:flex-row max-md:p-5 hover:border-[#01B9C5] hover:bg-teal-100 cursor-pointer">
          <div className="max-md:flex max-md:flex-col hidden">
          <span className="font-rubik uppercase text-[#687078] text-2xl pr-7">
            {data.name}
          </span>
          <p className="lg:none block text-[#2F4353]">Привести тело впорядок</p>
          </div>
          ...
          <p className="text-center max-md:hidden">Привести тело впорядок</p>
            <div className={`absolute transition-all right-2 top-[-40px] ${timeIsUp ? 'top-[-1000px] opacity-0 duration-1000 ease-in-out' : '' }`}>
